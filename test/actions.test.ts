...
test("preflight refuses a markdown below the seller's floor price", async () => {
  const { repo } = await rig();
  const l = (await repo.listing(PINNED))!; // $412.00, floor $355.00
  const r = preflight("markdown_price", l, { newPriceCents: 34000 }, await showBudgetContext(repo, NO_BUDGET_PRESSURE));
...
test("preflight refuses a markdown beyond the configured discount cap", async () => {
  const { repo } = await rig();
  const l = (await repo.listing(PINNED))!;
  // $360 clears the $355 floor but is 12.6% off — inside the cap.
  const inside = preflight("markdown_price", l, { newPriceCents: 36000 }, await showBudgetContext(repo, NO_BUDGET_PRESSURE));
  assert.equal(inside.ok, true);
  // Raise the floor scenario: a 20% cut would be $329.60, below floor AND over cap.
  const outside = preflight("markdown_price", l, { newPriceCents: 32960 }, await showBudgetContext(repo, NO_BUDGET_PRESSURE));
  assert.equal(outside.ok, false);
  assert.ok(outside.checks.some((c) => !c.ok && c.name.includes("max discount")));
});

test("preflight refuses negative stock and implausible jumps", async () => {
  const { repo } = await rig();
  const l = (await repo.listing(PINNED))!;
  assert.equal(preflight("adjust_stock", l, { newQty: -1 }, await showBudgetContext(repo, NO_BUDGET_PRESSURE)).ok, false);
  assert.equal(preflight("adjust_stock", l, { newQty: 400 }, await showBudgetContext(repo, NO_BUDGET_PRESSURE)).ok, false);
  assert.equal(preflight("adjust_stock", l, { newQty: 3 }, await showBudgetContext(repo, NO_BUDGET_PRESSURE)).ok, true);
});

test("a committed markdown lands on BOTH sides and bumps the listing version", async () => {
  const { repo, exec, market, listingWrites } = await rig();
  const before = (await repo.listing(PINNED))!;

  const a = await exec.propose("markdown_price", PINNED, { newPriceCents: 37000 }, "Mark down to $370.00", "4 discount asks in 3 minutes");
  ...
  const after = (await repo.listing(PINNED))!;
...
test("rollback restores both sides from the preflight snapshot", async () => {
  const { repo, exec, market } = await rig();
  const before = (await repo.listing(PINNED))!;

  const a = await exec.propose("markdown_price", PINNED, { newPriceCents: 37000 }, "Mark down to $370.00", "test");
  await exec.approve(a.id);
  const rolled = await exec.rollback(a.id);

  assert.equal(rolled.status, "rolled_back");
  assert.equal(rolled.undoableUntil, null);
  assert.equal((await repo.listing(PINNED))!.priceCents, before.priceCents, "local price restored");
  ...
  assert.ok((await repo.listing(PINNED))!.version > before.version + 1);
});
...
test("a marketplace apply failure leaves NOTHING changed", async () => {
  ...
  assert.equal((await repo.listing(PINNED))!.priceCents, before.priceCents, "local untouched");
  assert.equal((await repo.listing(PINNED))!.version, before.version, "no phantom version bump");
...
test("a remote edit under us is caught at reserve, before anything is written", async () => {
  ...
  assert.equal((await repo.listing(PINNED))!.priceCents, 41200, "our mirror is untouched");
});

test("approving twice does not apply the markdown twice", async () => {
  const { repo, exec } = await rig();
  const a = await exec.propose("markdown_price", PINNED, { newPriceCents: 37000 }, "Mark down to $370.00", "test");

  const first = await exec.approve(a.id);
  const versionAfterFirst = (await repo.listing(PINNED))!.version;
  const second = await exec.approve(a.id);

  assert.equal(first.status, "committed");
  assert.equal(second.status, "committed");
  assert.equal((await repo.listing(PINNED))!.priceCents, 37000);
  assert.equal((await repo.listing(PINNED))!.version, versionAfterFirst, "the second approval was a no-op");
});

test("the same intent at the same version produces the same idempotency key", async () => {
...
test("swap_pinned moves the pin on both sides and unpins the previous lot", async () => {
  ...
  const a = await exec.propose("swap_pinned", target, {}, "Pin the Panda Dunks", "chat is asking about them");
  const committed = await exec.approve(a.id);

  assert.equal(committed.status, "committed");
  assert.equal((await repo.listing(target))!.pinned, true);
  assert.equal((await repo.listing(PINNED))!.pinned, false);
...
test("the audit chain records the whole lifecycle and verifies", async () => {
  const { exec, audit } = await rig();
  const a = await exec.propose("markdown_price", PINNED, { newPriceCents: 37000 }, "Mark down to $370.00", "test");
  await exec.approve(a.id);
  await exec.rollback(a.id);

  const kinds = (await audit.list()).map((e) => e.kind);
...
test("tampering with a historical audit row is detected", async () => {
  const { d, exec, audit, showId } = await rig();
  const a = await exec.propose("markdown_price", PINNED, { newPriceCents: 37000 }, "Mark down to $370.00", "test");
  await exec.approve(a.id);
  assert.equal((await audit.verify()).ok, true);

  // Rewrite history: make the committed markdown look like it was to $390.
  await d.query("UPDATE audit SET summary = $1 WHERE show_id = $2 AND seq = 2",
    ["Mark down to $390.00", showId]);
...
test("the audit hash depends on every field, with no separator collisions", async () => {
...
test("a failed action is recorded in the audit, not swallowed", async () => {
  const { exec, audit, market } = await rig();
  market.failNext();
  const a = await exec.propose("markdown_price", PINNED, { newPriceCents: 37000 }, "Mark down to $370.00", "test");
  await exec.approve(a.id);
...
test("an action whose preflight failed can never be committed", async () => {
  const { repo, exec } = await rig();
  const before = (await repo.listing(PINNED))!;
  const a = await exec.propose("markdown_price", PINNED, { newPriceCents: 1000 }, "Mark down to $10.00", "absurd");
