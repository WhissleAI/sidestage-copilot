...
Read on 2026-09-15 against the code, not the other way round. ✓ built and exercised,
~ partial and named, ✗ not built and named.

| PRD requirement | Status | Where, or why not |
|---|---|---|
| §2 buyer question → claim-structured reply → guardrails against **current** state → card with provenance → one-keystroke send | ✓ | `src/pipeline/pipeline.ts`, `src/guardrails/`; exercised live on a real eBay Live show: admitted → allowed → card in <14 s with 8 cited facts. The draft streams to the card as it forms; the guards judge the complete draft |
...
| §2 "every cited fact is re-checked against live state before send" | ✓ | `Pipeline.send()`: a blocked proposal is refused (HTTP 409) whatever the client asks, and an operator-edited draft is re-run through the whole chain against its grounding facts and the listings as they stand now; `verdictAtSend` / `guardsAtSend` land in the audit entry |
| §2 operational problem → bounded, reversible action → preflight → approve → commit → hash-chained audit → undo | ✓ against the mock, ~ against eBay | `src/actions/`; two-phase against `MockMarketplace` by default. The real Sell Inventory adapter exists (`src/actions/marketplace/ebay.ts`: value-based lock, `bulk_update_price_quantity`, withdraw-not-delete, 429/503 backoff) and a show is armed to it per show once its owner has connected eBay — a show attached from a stream is read-only unless the eBay username behind the seller's consent matches its seller handle (the only proof accepted), so preflight refuses every write on anyone else's show — and **no action has yet committed through the eBay adapter on a live show the account owns**. The undo window is enforced on rollback; a failed compensation fails the action loudly naming the listing |
| §2 "no write is proposed that cannot be undone" | ~ | `push_listing` and `swap_pinned` are local no-ops on the eBay adapter (eBay Live exposes no on-screen API); their undo is local too. `end_listing` withdraws rather than deletes so it can be republished |
| §3 one seller, one show, one catalog | ~ | The product also monitors and prepares **other** sellers' shows (read-only), added on request. Multi-seller is real: every account registers, and a show belongs to the account that attached it (404 to anyone else; lists and the SSE show list cut per caller). Guard **settings** are still process-wide — the last seller to attach arms Layer B for everyone; per-show policy is next |
| §4 every metric computed per show | ✓ | `src/shows/prdMetrics.ts`, on every report; aggregated across the caller's shows on Analytics → Overview |
...
| §5 five-rung ladder, promotion criteria checkable against own numbers | ✓ | `src/autonomy/`; criteria rendered with progress on Analytics → Autonomy |
| §5 "L4 stays locked while writes hit a mock" | ~ | Settings refuse `L4_AUTO_ACT` as a *starting* rung and the ladder auto-commits only `markdown_price` / `adjust_stock` after preflight — but `POST /api/autonomy` will move a show to L4 whatever its write target. The lock is a rule the seller keeps, not one the code checks against the write target |
| §5 a `block` never auto-sends; price and discount never on the L3 allow-list | ✓ | `src/autonomy/ladder.ts` |
| §6 pilot instrumentation — span breakdown, evidence, guard verdicts on every proposal | ✓ (since 2026-09-15) | persisted in `reply_proposals`, readable per reply in the console inspector. The INSERT did not run before 2026-09-15, so earlier reports carry no drafted replies |
| §6 "every send, edit and dismiss is logged with the guardrail verdict at send time" | ✓ | `reply_sent` audit entries carry `edited`, `verdictAtSend`, `guardsAtSend`, attributed to `seller:<handle>` |
| Delivery of a sent reply to the marketplace chat | ✗ | eBay Live exposes no chat-post API. A "sent" reply is recorded and audited, not delivered; the seller pastes it. Stated in the README |
| Accounts and attribution | ✓ | `src/auth/accounts.ts`: register / login / logout / me, scrypt; every `/api/*` route except health, auth, eBay's callbacks and the audio bridge needs a session; every non-GET needs a seller. No guest kind any more |
| Tenancy | ✓ (with a documented legacy) | `owner_account_id` set at attach; a preHandler in `src/api/routes.ts` answers 404 for any show-bound route to a non-owner; `/api/shows`, `/api/reports`, `/api/home`, `/api/cost`, analytics and catalogs are scoped; the SSE `shows` list is cut per client. Rows older than ownership have no owner and stay visible to everyone. `/api/shows/prepared` is workspace-wide on purpose |
| Seller connects their own marketplace | ✓ | eBay OAuth consent on a **production** keyset, `state` pinned to the account, tokens sealed at rest under `EBAY_TOKEN_KEY`; Marketplace Account Deletion notices honoured only after eBay's signature over the raw body verifies. Sold comps need the Marketplace Insights grant the production keyset lacks, so comparables are asking prices and say so |
...
| One Whissle agent per show, and its lifecycle | ✓ | `src/llm/streamAgent.ts`; retired a day after the show's report by `src/llm/agentGc.ts`; re-preparing retires the old agent; the 50-agent workspace cap triggers one retirement pass and one retry |
| Hosted | ✓ | Console at https://sidestage.whissle.ai (Vercel); backend at https://35-173-35-240.sslip.io (one t3.small, Docker Compose, Caddy TLS). Discover needs the house eBay session read through `EBAY_DISCOVERY_PROXY`; attaching by link, reports, analytics and eBay consent run without it |
